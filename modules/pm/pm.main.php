...
Hooks=pm.first,pm.main,pm.loop,pm.tags
[END_SED]
==================== */
...
$pm_perpage = isset($cfg['maxpmsperpage']) ? (int)$cfg['maxpmsperpage'] : 15;
if ($pm_perpage < 1) {
	$pm_perpage = 15;
}
...

{
	...
	if ($f == 'archives') {
		...
		$sql = sed_sql_query("SELECT * FROM $db_pm
			...
			ORDER BY pm_date DESC LIMIT $d," . $pm_perpage);
		$title .= " " . sed_link(sed_url("pm", "f=archives"), $L['pm_archives']);
	...
	$pm_totalpages = ceil($totallines / $pm_perpage);
	$pm_currentpage = ceil($d / $pm_perpage) + 1;

	$pm_pagination = sed_pagination(sed_url("pm", "f=" . $f), $d, $totallines, $pm_perpage);
	list($pm_pageprev, $pm_pagenext) = sed_pagination_pn(sed_url("pm", "f=" . $f), $d, $totallines, $pm_perpage, TRUE);
}
...
while ($row = sed_sql_fetchassoc($sql) and ($jj < $pm_perpage)) {
	$jj++;
