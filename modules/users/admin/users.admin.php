...
switch ($n) {
	case 'add':
		...
		sed_redirect(sed_url("admin", "m=users", "", true), false, ['msg' => '301']);
		exit;
		break;
	...
	case 'edit':
		...
		if ($a == 'syncrights' && $g > 0 && sed_xg()) {
			...
			sed_redirect(sed_url("admin", "m=users&n=edit&g=" . $g, "", true), false, ['msg' => '917']);
			exit;
		}
		...
		if ($a == 'update') {
			...
			sed_cache_clear('sed_groups');
			sed_redirect(sed_url("admin", "m=users", "", true), false, ['msg' => '917']);
			exit;
		} elseif ($a == 'delete' && $g > 5) {
			...
			sed_auth_clear('all');
			sed_cache_clear('sed_groups');
			sed_redirect(sed_url("admin", "m=users", "", true), false, ['msg' => '302']);
			exit;
		}
