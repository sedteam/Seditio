...
[BEGIN_SED]
File=modules/gallery/gallery.details.php
Version=186
Updated=2026-sep-21
Type=Module
Author=Seditio Team
...
/* === Hook === */
foreach (sed_getextplugins('gallery.details.first') as $pl) {
		include $pl;
	}
/* ===== */
...
if ($pfs['pfs_imgsize'][0] > $cfg['th_imgmaxwidth']) {
	if (!file_exists($cfg['res_dir'] . $pfs['pfs_file'])) {
		sed_image_process(
			$pfs['pfs_fullfile'],
			$cfg['res_dir'] . $pfs['pfs_file'],
			$cfg['th_imgmaxwidth'],
			0,
			$cfg['th_keepratio'],
			'resize',
...
/* === Hook === */
foreach (sed_getextplugins('gallery.details.main') as $pl) {
		include $pl;
	}
/* ===== */

require(SED_ROOT . "/system/header.php");
$t = new XTemplate(sed_skinfile(array('gallery', 'details')));
...
/* === Hook === */
foreach (sed_getextplugins('gallery.details.tags') as $pl) {
		include $pl;
	}
/* ===== */
