...
Author=Seditio Team
Description=Forums
Hooks=forums.posts.first,forums.posts.newpost.first,forums.posts.newpost.done,forums.posts.delete.first,forums.posts.delete.done,forums.posts.emptytopicdel,forums.posts.main,forums.posts.loop,forums.posts.newpost.tags,forums.posts.tags
[END_SED]
==================== */
...
if ($a == 'newpost' && $usr['auth_write']) {
	...
	if ($row = sed_sql_fetchassoc($sql)) {
		if (isset($cfg['antibumpforums']) && !empty($cfg['antibumpforums']) && (($usr['id'] == 0 && $row['fp_posterid'] == 0 && $row['fp_posterip'] == $usr['ip']) || ($row['fp_posterid'] > 0 && $row['fp_posterid'] == $usr['id']))) {
			sed_die();
		}
	} else {
		sed_die();
...
if (!empty($p)) {
	...
	$d = $maxtopicsperpage * floor($postsbefore / $maxtopicsperpage);
}
...
$sql = sed_sql_query("SELECT p.*, u.user_text, u.user_maingrp, u.user_avatar, u.user_photo, u.user_signature, " . $sql_extra . "	
	   ...
	   u.user_postcount
		...
		WHERE fp_topicid='$q'
		ORDER BY fp_id LIMIT $d, " . $maxtopicsperpage);

$nbpages = ceil($totalposts / $maxtopicsperpage);
$curpage = $maxtopicsperpage ? $d / $maxtopicsperpage : 0;
$notlastpage = (($d + $maxtopicsperpage) < $totalposts) ? TRUE : FALSE;
...
if (sed_module_active('polls') && $ft_poll > 0) {
	...
	if (sed_sql_numrows($sql5) > 0) {
		...
		if ($alreadyvoted) {
			...
			$ajax_send = sed_url("forums", "m=posts&q=" . $q . "&a=send&" . sed_xg() . "&poll=" . $ft_poll . "&ajax=1");
...
$antibump = (int)(isset($cfg['antibumpforums']) ? $cfg['antibumpforums'] : 0);
$allowreplybox = (!$antibump) ? TRUE : FALSE;
$allowreplybox = ($antibump && $lastposterid > 0 && $lastposterid == $usr['id'] && $usr['auth_write']) ? FALSE : TRUE;
