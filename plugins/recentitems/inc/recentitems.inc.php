...
function sed_get_latestcomments($limit, $mask)
  ...
  while ($row = sed_sql_fetchassoc($sql))
	...
	$res .= sprintf($mask, $lnk, $com_authorlink, sed_build_date($cfg['formatyearmonthday'], $row['com_date'], $cfg['plu_mask_comments_date']), sed_build_userimage($row['user_avatar']), $com_text);
    }
