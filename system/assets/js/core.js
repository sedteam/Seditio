...
const sedjs = {
  ...
  sedtabs(settings) {
    ...
    const initTabs = (config) => {
      ...
      const handleTabClick = function () {
        ...
        if (!mergedConfig.beforeSwitchCallback || mergedConfig.beforeSwitchCallback.apply(clickedTab, this.callbackArgs) !== false) {
      ...
      for (let i = 0; i < tabContainers.length; i++) {
        ...
        for (let j = 0; j < tabLinks.length; j++) {
          if (tabLinks[j].href.match(/#tab/)) {
            ...
            if (typeof mergedConfig.defaultTabIndex === 'string' && tabId === mergedConfig.defaultTabIndex) {
        ...
        for (let j = 0; j < tabs.length; j++) {
          const callbackArgs = [tabIds[j], tabs, tabContents, mergedConfig];

          tabs[j].tabLinks = tabs;
          tabs[j].tabContents = tabContents;
          tabs[j].callbackArgs = callbackArgs;

          tabs[j][`on${mergedConfig.eventType}`] = bindFunction.call(tabs[j], handleTabClick);
        }

        if (typeof mergedConfig.defaultTabIndex === 'number' && mergedConfig.defaultTabIndex >= 0) {
          switchTab.call(tabs[mergedConfig.defaultTabIndex], tabIds[mergedConfig.defaultTabIndex], tabs, tabContents, mergedConfig);
        }
      }
    };
  ...
  modal: {
    ...
    getviewpoint() {
      ...
      // Get scroll position
      this.scroll_top = ie ? body.scrollTop || docElement.scrollTop : window.pageYOffset !== undefined ? window.pageYOffset : 0;
      this.scroll_left = ie ? body.scrollLeft || docElement.scrollLeft : window.pageXOffset !== undefined ? window.pageXOffset : 0;

      // Get dimensions of the visible area
      this.docwidth = ie ? this.standardbody.clientWidth : /Safari/i.test(navigator.userAgent) ? window.innerWidth : Math.min(domclientWidth, window.innerWidth - 16);
      this.docheight = ie ? this.standardbody.clientHeight : window.innerHeight || 0;
    },
  ...
  genSEF(from, to, allow_slashes) {
    ...
    str = str.replace(new RegExp(`(ь|ъ)([${Vowel}])`, 'g'), 'j$2');
   ...
   */
  popover: {
    template: '<div class="sed-popover" style="display:none;" data-sed-popover>' + '<button type="button" class="sed-popover-close" data-sed-popover-close>×</button>' + '<div class="sed-popover-body"></div></div>',
    activePopover: null,
    ...
    init(containerEl) {
      ...
      for (let i = 0; i < triggers.length; i++) {
        ...
        const contentEl = targetId ? document.getElementById(targetId) : trigger.parentElement.querySelector('[data-sed-popover-content]');
  ...
  sortable(element, options) {
    ...
    function init() {
      if (settings.connectWith) {
        connectedLists = Array.prototype.slice.call(document.querySelectorAll(settings.connectWith));
      }
      updateItems();
      bindEvents();
      setupDynamicCheck();
    ...
    function bindEvents() {
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const dragElement = settings.handle ? item.querySelector(settings.handle) || item : item;
    ...
    function movePlaceholder(target, clientX, clientY) {
      ...
      const shouldInsertBefore = isVertical ? clientY < rect.top + rect.height / 2 : clientX < rect.left + rect.width / 2;
