...
switch ($m)
	{
		case "comments" :		
			...
			if ($cfg['disable_comments'] || $cfg['disable_rsscomments']) { $i=0; break; }
		...
		case "forums" :
			...
			if ($cfg['disable_forums'] || $cfg['disable_rssforums']) { $i=0; break; }
					...
					if (sed_auth('forums', $row['fp_sectionid'], 'R'))
						{
							...
							if (!$row['fp_text_ishtml'] && $cfg['textmode']=='html')
								{
								$sql3 = sed_sql_query("UPDATE $db_forum_posts SET fp_text_ishtml=1, fp_text='".sed_sql_prep($row['fp_text'])."' 
		...
		default : 
			...
			if ($cfg['disable_page'] || $cfg['disable_rsspages']) { $i=0; break; }
			...
			while ($row = sed_sql_fetchassoc($sql))
				...
				if (!$row['page_text_ishtml'] && $cfg['textmode']=='html')
					{
					 $sql2 = sed_sql_query("UPDATE $db_pages SET page_text_ishtml=1, page_type=1, page_text='".sed_sql_prep($row['page_text'])."', 
											page_text2='".sed_sql_prep($row['page_text2'])."' WHERE page_id=".$row['page_id']);
					}										
