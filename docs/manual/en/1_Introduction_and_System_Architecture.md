...
For a detailed guide on creating, structuring, registering hooks, and plugin call modes, see [Chapter 8. Plugin Architecture and Development](8_Plugin_Architecture_and_Development.md).
...
For a detailed technical description of all 12 input filters, the security token generation algorithm, and protection against SQL injection, see [Chapter 10. Security and Performance Optimization](10_Security_and_Performance_Optimization.md).
...
To study the design of the access control system, the database table structure, caching rights in the `user_auth` field, and the authorization check process in PHP code in detail, refer to [Chapter 4. User System and Access Control (ACL)](4_User_System_and_Access_Control_ACL.md).
