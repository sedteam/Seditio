...
### 4.5.4. Регистрация и управление профилем (в версии 186)
* **Безопасная обработка регистрации:** Контроллер регистрации учетных записей (`modules/users/users.register.php`) строго ограничен методом `POST`. Прямые обращения методом `GET` не инициируют процесс валидации, предотвращая появление ложных ошибок.
* **Совместимость с PHP 8.1+:** Входные строковые параметры формы санитизируются до передачи в функции `mb_strtolower()`, `str_replace()` и `mb_strlen()`, исключая фатальные сбои при передаче значений `null`.
* **Безопасная инициализация прав:** При авторизации переменная `$user_auth` гарантированно инициализируется пустым массивом при отсутствии данных, защищая код от нежелательных предупреждений интерпретатора.
* **Современный виджет загрузки аватара и фото:** В личном кабинете пользователя (`users.profile.tpl`) поля загрузки аватара, фото и подписи переведены на компонент `sedjs.imageUpload` в режиме плиток (`tiles`), обеспечивающий предпросмотр и сохранение выбранных файлов при промежуточной валидации формы.
